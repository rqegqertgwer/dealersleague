...
<script>
/*jQuery('.size-thumbnail').each(function(data) {
	if(jQuery('.flex-control-nav').children().length <= 5) {
...
jQuery('.arrow_up').click(function() {
	jQuery('.flex-control-nav').animate({ marginTop: "-=100"}, 1000); 
	console.log(jQuery("#wpgis-gallery .flex-control-nav").offset());
	if(jQuery("#wpgis-gallery").position.top <= 824.640625){
       console.log('success');
	}
 });
